...
const allQuestions = [
  ...
  },
  {
    id: 6,
    question:
      '現在使われているタクシーメーターは、どうやって料金を算出しているでしょうか？',
    answer: '距離と時間',
  ...
  },
  {
    ...
    explanation:
      '　ガスメーターは、体積をはかることができます。おうちのガスメーターを見てみましょう。',
  },
  {
    id: 9,
...
const Quiz = (props) => {
  ...
  const [detailedExplanationsVisible, setDetailedExplanationsVisible] =
    useState({});
  ...
  const handleSubmitAnswer = (questionId) => {
    ...
    const allAnswered =
      Object.keys(answeredQuestions).length + 1 === questions.length;
    if (allAnswered) {
          ...
          {score >= 4 ? (
            ...
            <>
              <p className='failure'>
                <TextWithRuby
                  text={
                    '挑戦してくれてありがとう！「計量はかせ」目指して、また挑戦してね！'
                  }
                />
              </p>
        ...
        <div className='QuizContainer' ref={scrollRef}>
          {questions.map((question, questionIndex) => (
            <>
              <div
                key={question.id}
                className='question-container sticker-box green text'
              >
                <TextWithRuby
                  text={`第${questionIndex + 1}問：${question.question}`}
                />
                <br />
                {question.questionImg && (
                  <img src={question.questionImg} alt='画像' />
                )}
                ...
                <div className='tab-container'>
                  <button
                    className={`tab-btn ${
                      activeTab[question.id] === 'feedback' ? 'active' : ''
                    }`}
                    onClick={() => handleTabChange(question.id, 'feedback')}
                    disabled={!selectedOptions[question.id]}
                  >
                    {feedbacks[question.id] ? (
                      <TextWithRuby text={`解説`} />
                    ) : (
                      <TextWithRuby text={`回答`} />
                    )}
                  </button>
                  <button
                    className={`tab-btn hint-btn ${
                      activeTab[question.id] === 'hint' ? 'active' : ''
                      ...
                      {feedbacks[question.id] && (
                        <>
                          <p className='explanation'>
                            {question.explanationImg1 && (
                              <img src={question.explanationImg1} alt='画像' />
                            )}
                          ...
                          {detailedExplanationsVisible[question.id] && (
                            <>
                              ...
                              <p className='detailed-explanation'>
                                ...
                                />
                                {question.explanationImg2 && (
                                  <img
                                    src={question.explanationImg2}
                                    alt='画像'
                                  />
                                )}
                              </p>
